...
void loop()
{
    ...
    {
				if(pixels_temp[i] <= MINTEMP) 
					colorTemp = MINTEMP;
				if(pixels_temp[i] >= MAXTEMP) 
					colorTemp = MAXTEMP;
				uint8_t colorIndex = map(pixels_temp[i], MINTEMP, MAXTEMP, 0, 255);
				Tft.fillRectangle(TFT_PIXELS_NUM*(i/8), YStart+TFT_PIXELS_NUM*(i%8), TFT_PIXELS_NUM, TFT_PIXELS_NUM, camColors[colorIndex]);
    }
}
/*********************************************************************************************************
  END FILE
